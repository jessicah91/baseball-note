...
function parseMatchup(rawText) {
  ...
  const scoreOrState = middle.match(/(\d+\s*:\s*\d+\s*(?:Final(?:\/\d+)?|Top \d+(?:st|nd|rd|th)|Bot(?:tom)? \d+(?:st|nd|rd|th))|(?:Final(?:\/\d+)?|Top \d+(?:st|nd|rd|th)|Bot(?:tom)? \d+(?:st|nd|rd|th))|\d{1,2}:\d{2}(?:am|pm))/i);

  if (scoreOrState) {
    status = scoreOrState[0].trim();
  }
...
function parseSchedule(html, selectedTeam) {
  ...
  for (const part of parts) {
    const headingMatch = part.match(/([^<]+)<\/h3>/i);
    if (!headingMatch) continue;
    const dateLabel = decodeHtml(headingMatch[1]);

    const games = [];
    const linkMatches = [...part.matchAll(/<a[^>]*href="\/games\/[^\"]+"[^>]*>(.*?)<\/a>/gis)];
    for (const match of linkMatches) {
      const game = parseMatchup(match[1]);
      if (!game) continue;
